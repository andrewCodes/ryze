...
// to prevent burger icon scrolling when burger menu is open...

const burger = document.querySelector('.burger-menu__trigger'); // grab the trigger
const docBody = document.body; // grab the <body>
const closeNav = document.querySelector('.nav-close'); // grab the .nav-close element

burger.onclick = function(){ // when click burger icon (trigger) to open / close the menu...
  console.log('clicked');

  if(docBody.classList.contains('scroll')){ // if <body> has class .scroll (it does on page load i.e. when burger menu is closed)...
    docBody.style.overflow = 'hidden'; // prevent window from scrolling (the menu will now be open)
    docBody.classList.remove('scroll'); // remove .scroll
  } else { // if the <body> does not have .scroll it means the menu is open so on click we...
    docBody.style.overflow = 'scroll'; // let the window scroll again (the menu will now be closed)
    docBody.classList.add('scroll'); // add back .scroll in preparation for next click
  }
};

closeNav.onclick = function(){ // need to add this as the menu can also be closed by clicking anywhere outside the nav links... exactly the same as code block above
  console.log('clicked other');

  if(docBody.classList.contains('scroll')){
    docBody.style.overflow = 'hidden';
    docBody.classList.remove('scroll');
  } else {
    docBody.style.overflow = 'scroll';
    docBody.classList.add('scroll');
  }
};
